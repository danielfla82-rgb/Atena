...
Aumente a carga horária em **resolução de questões** e reduza a leitura passiva de teoria. O momento exige prática deliberada.
  ...
  const renderNewsText = useCallback((text: string) => {
    return text.split('\n').map((line, index) => {
        ...
        // H2 (##)
        if (trimmedLine.startsWith('##')) {
             return <h2 key={index} className="text-xl font-bold text-green-400 mt-6 mb-3 border-b border-slate-300 dark:border-slate-700 pb-2">{parseInline(trimmedLine.replace(/^##\s*/, ''))}</h2>;
        }
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6 h-full flex flex-col">
       <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-6 flex-shrink-0">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">Alvo: <span className="text-green-400 font-bold uppercase tracking-wider">{config.targetRole}</span></p>
        </div>
