...
// Configuração dos Ritmos de Estudo
const PACE_SETTINGS: Record<string, { hours: number, blocks: number }> = {
    'Iniciante': { hours: 10, blocks: 15 }, // 10h = 600min / 40min = 15 blocos
    'Básico': { hours: 20, blocks: 30 }, // 20h = 1200min / 40min = 30 blocos
    'Intermediário': { hours: 30, blocks: 45 }, // 30h = 1800min / 40min = 45 blocos
    'Avançado': { hours: 44, blocks: 66 } // 44h = 2640min / 40min = 66 blocos
};

export const Setup: React.FC = () => {
  const { notebooks, config, updateConfig, moveNotebookToWeek, addNotebook, editNotebook } = useStore();
  
  // View State: 'timeline' (Kanban) or 'calculator' (Cycle Builder)
  const [viewMode, setViewMode] = useState<'timeline' | 'calculator'>('timeline');

  const [searchTerm, setSearchTerm] = useState('');
  
  // UX Improvement: Charts collapsed by default to focus on Kanban
  ...
  // --- DATA PROCESSING FOR CHARTS & METRICS ---
  ...
  const allocationData = useMemo(() => {
    ...
    return Object.keys(data)
      ...
      .slice(0, 8); // Top 8 disciplines
  }, [notebooks]);

  // Calculate Total Allocated Blocks in Weeks
  ...
  const libraryNotebooks = useMemo(() => {
    return notebooks.filter(nb => 
        !nb.weekId && // Only show backlog items in sidebar
  ...
  const getWeekDateRange = (weekIndex: number, startDateStr?: string) => {
    ...
    return {
        ...
        fullDate: `${start.toLocaleDateString('pt-BR')} até ${end.toLocaleDateString('pt-BR')}`,
        isPast
    };
  };

  const weeks = Array.from({ length: config.weeksUntilExam || 4 }, (_, i) => {
  ...
  const onDrop = (e: React.DragEvent, targetWeekId: string | null, isPast: boolean) => {
    if (isPast) {
        alert("Você não pode alterar o planejamento de semanas que já passaram.");
        return;
    }
      ...
      if (origin === 'library') {
        ...
        if (original) {
            addNotebook({
                ...original,
                weekId: targetWeekId,
                isWeekCompleted: false // Reset completed state when adding from library
            });
        }
      } else {
  ...
  const handleManualReview = () => {
    ...
    const nextDate = calculateNextReview(acc, formData.relevance, formData.trend);
    
    if (editingId) {
       editNotebook(editingId, {
          accuracy: acc,
          lastPractice: new Date().toISOString(),
          nextReview: nextDate.toISOString()
       });
       alert(`Revisão registrada! Próxima revisão: ${nextDate.toLocaleDateString()}`);
    ...
    <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-slate-950">
      ...
      <aside className="w-72 flex-shrink-0 border-r border-slate-800 bg-slate-900/30 flex flex-col z-20 hidden md:flex">
          <div className="p-4 border-b border-slate-800 space-y-3">
            <h2 className="text-xs font-bold text-slate-400 flex items-center gap-2 uppercase tracking-wider">
               <Layout size={14} className="text-emerald-500" />
               Matérias não alocadas
            </h2>
            ...
            <div className="relative">
                ...
                <input 
                    ...
                    className="w-full bg-slate-950 border border-slate-700 rounded-lg py-2 pl-9 pr-3 text-xs text-white focus:border-emerald-500 focus:outline-none placeholder-slate-600 transition-colors"
                />
            </div>
          ...
          <div className="p-2 space-y-2 overflow-y-auto flex-1 custom-scrollbar">
            ...
            {libraryNotebooks.length === 0 && (
              <div className="flex flex-col items-center justify-center py-10 text-slate-600 gap-2 opacity-50">
                 <Settings2 size={24} />
                 <span className="text-xs text-center">Nenhum caderno no backlog.<br/>Crie um novo em "Banco de Dados".</span>
              </div>
            )}
          </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-950 relative">
         ...
         <header className="flex flex-col lg:flex-row items-center justify-between gap-4 px-6 py-4 border-b border-slate-800 bg-slate-900/90 backdrop-blur-xl sticky top-0 z-30 shadow-lg">
            <div className="flex items-center gap-4 w-full lg:w-auto lg:flex-1">
                 <div className="flex flex-col">
                    ...
                    <div className="flex items-center gap-2 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 hover:border-slate-700 transition-colors">
                        <Calendar size={14} className="text-emerald-500" />
                        <input 
                            type="date"
                            value={config.startDate || ''}
                            onChange={(e) => updateConfig({...config, startDate: e.target.value})}
            ...
            <div className="w-full lg:w-auto flex justify-center order-3 lg:order-2">
                <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800 shadow-xl">
                    <button 
                        ...
                        className={`px-4 md:px-6 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 whitespace-nowrap ${viewMode === 'timeline' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
                    >
                        <GanttChartSquare size={16} /> Visão Tática
                    </button>
                    <button 
                        onClick={() => setViewMode('calculator')}
                        className={`px-4 md:px-6 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 whitespace-nowrap ${viewMode === 'calculator' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
                    >
                        <Calculator size={16} /> Ciclo
                    </button>
                </div>
            </div>
            ...
            <div className="flex items-center gap-3 w-full lg:w-auto lg:flex-1 justify-between lg:justify-end order-2 lg:order-3">
                 <div className="relative group w-full md:w-auto max-w-[200px]">
                    ...
                    <div className="relative flex items-center px-4 py-2 gap-3 cursor-pointer">
                        <div className="flex items-center justify-center bg-slate-900 p-1.5 rounded-lg border border-slate-700 text-emerald-500">
                             <Timer size={16} />
                        </div>
                        <div className="flex flex-col flex-1 min-w-0">
                             ...
                             <div className="flex items-center justify-between">
                                <select 
                                    value={config.studyPace}
                                    onChange={(e) => updateConfig({...config, studyPace: e.target.value as any})}
                                    className="bg-transparent text-white text-xs font-bold outline-none cursor-pointer hover:text-emerald-400 appearance-none w-full z-10"
                                >
                 ...
                 <button 
                    ...
                    className={`h-[42px] w-[42px] flex items-center justify-center rounded-xl transition-all border flex-shrink-0 ${showStats ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/50' : 'bg-slate-800 text-slate-400 border-slate-700 hover:text-white hover:bg-slate-700'}`}
                    title="Alternar Estatísticas"
                >
                    <BarChart3 size={18} />
                </button>
            </div>
         ...
         {viewMode === 'timeline' ? (
             <div className="flex flex-col h-full overflow-hidden">
                 {/* INTELLIGENT CHARTS AREA (Toggleable) */}
                 <div 
                    className={`overflow-hidden transition-all duration-300 ease-in-out bg-slate-900 border-b border-slate-800 flex-shrink-0
                        ${showStats ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0 border-none'}
                    `}
                 >
                    <div className="p-4 h-64 flex gap-6">
                        <div className="flex-1">
                            <h3 className="text-xs font-bold text-slate-400 mb-2 uppercase flex items-center gap-2">
                                ...
                                Distribuição de Matérias no Planejamento
                            </h3>
                        ...
                        <div className="w-64 border-l border-slate-800 pl-6 flex flex-col justify-center gap-4">
                            <div>
                                <p className="text-xs text-slate-500 uppercase font-bold">Total Alocado (Global)</p>
                                <p className="text-lg font-bold text-white truncate">{totalAllocatedBlocks} Blocos</p>
                            </div>
                            <div>
                                ...
                                <div className="w-full bg-slate-800 h-2 rounded-full mt-1 overflow-hidden">
                                    <div 
                                        ...
                                        style={{width: `${(weeks.filter(w => w.isPast).length / weeks.length) * 100}%`}}
                                    ></div>
                                </div>
                                <p className="text-xs text-right text-slate-400 mt-1">
                                    {weeks.filter(w => w.isPast).length} / {weeks.length} Semanas
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        return (
                            ...
                            className={`
                                ...
                                ${week.isPast 
                                    ...
                                    : 'bg-slate-900 border-slate-800 shadow-2xl hover:border-slate-700 hover:shadow-emerald-900/10'}
                            `}
                            ...
                            {/* Week Header */}
                            <div className={`
                                p-4 rounded-t-2xl border-b flex flex-col gap-3 z-10 relative
                                ${week.isPast 
                                    ? 'bg-slate-950/30 border-slate-800/50 text-slate-600' 
                                    : 'bg-slate-900 border-slate-700 text-slate-200'}
                            `}>
                                <div className="flex justify-between items-start">
                                    <div>
                                        <span className="font-black block text-base flex items-center gap-2 text-white">
                                            SEMANA {week.index}
                                            {week.isPast && <Lock size={14} />}
                                        </span>
                                        <span className={`text-[10px] font-bold uppercase tracking-widest ${week.isPast ? 'line-through decoration-slate-600 opacity-50' : 'text-slate-500'}`}>
                                            {week.label}
                                        </span>
                                    </div>
                                    <div className="flex flex-col items-end">
                                        <span className={`text-lg font-black ${isOverloaded ? 'text-red-400' : isComplete ? 'text-emerald-400' : 'text-white'}`}>
                                            {blocksCount}
                                        </span>
                                ...
                                {/* Weekly Load Indicator & Missing Blocks */}
                                {!week.isPast && (
                                    <div className="space-y-2">
                                        <div className="w-full h-2 bg-slate-950 rounded-full overflow-hidden flex relative group border border-slate-800">
                                            <div 
                                                className={`h-full transition-all duration-500 ${isOverloaded ? 'bg-red-500' : 'bg-gradient-to-r from-emerald-600 to-emerald-400'}`} 
                                                style={{ width: `${loadPercentage}%` }}
                                            ></div>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            {missingBlocks > 0 ? (
                                                <span className="text-[10px] text-amber-500 font-bold flex items-center gap-1 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">
                                                    <AlertCircle size={10} /> Meta: +{missingBlocks} blocos
                                                </span>
                                            ) : (
                                                <span className="text-[10px] text-emerald-500 font-bold flex items-center gap-1 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                            ...
                            <div className="p-3 space-y-2 overflow-y-auto flex-1 custom-scrollbar relative bg-slate-900/50">
                                {week.isPast && (
                                    <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/diagmonds-light.png')] opacity-10 pointer-events-none z-0"></div>
                                )}
                                ...
                                {weekNotebooks.length === 0 && !week.isPast && (
                                <div className="h-full flex flex-col items-center justify-center text-slate-700 text-xs italic opacity-50 border-2 border-dashed border-slate-800 rounded-xl m-2 bg-slate-950/50 min-h-[100px]">
                                    Arraste matérias aqui
                                </div>
                                )}
            
                                {week.isPast && weekNotebooks.length === 0 && (
                                    <div className="h-full flex items-center justify-center text-slate-800 text-sm uppercase font-black tracking-widest rotate-[-5deg]">
                                        Concluída
                                    </div>
                                )}
                            </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
            ...
            <form onSubmit={handleSave} className="overflow-y-auto p-6 space-y-6 custom-scrollbar">
              {/* Form Content kept same as before for functionality */}
              <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wider">Link do Caderno</label>
                        <div className="relative">
                  ...
                  <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 flex flex-col md:flex-row items-center gap-4">
                      <div className="flex-1 w-full">
                         ...
                         <input 
                            ...
                            className="w-full bg-slate-900 border border-slate-600 rounded-lg p-2 text-white font-mono text-center font-bold text-lg focus:border-emerald-500 outline-none"
                         />
                      </div>
                      <div className="flex-1 w-full">
                         <label className="block text-[10px] font-bold text-slate-500 mb-2 uppercase opacity-0 md:opacity-100">Ações Rápidas</label>
                         <button 
                            type="button"
                            onClick={handleManualReview}
                            className="w-full py-2.5 px-4 rounded-lg flex items-center justify-center gap-2 font-bold text-sm bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg transition-all"
                         >
                ...
                <div>
                    ...
                    <div className="relative">
                        ...
                        <input 
                            ...
                            onChange={e => handleChange('obsidianLink', e.target.value)} 
                            className="w-full bg-slate-800 border border-slate-700 rounded-lg p-3 pl-10 text-white outline-none focus:border-emerald-500" 
            ...
            <div className="p-6 border-t border-slate-800 bg-slate-900 flex gap-4">
                ...
                <button type="button" onClick={handleSave} className="flex-1 bg-emerald-600 text-white py-3 rounded-xl hover:bg-emerald-500 font-bold shadow-lg shadow-emerald-900/20 transition-all">
                    Salvar Alterações
                </button>
            </div>
          </div>
...
// --- SUB-COMPONENT: CYCLE CALCULATOR ---
...
const CycleCalculator: React.FC<{ paceTarget: { hours: number, blocks: number } }> = ({ paceTarget }) => {
    ...
    const handleAddItem = (e: React.FormEvent) => {
        e.preventDefault();
        if(!newName) return;
        setItems([...items, { id: Math.random().toString(36), name: newName, weight: newWeight }]);
        setNewName('');
    };
    ...
    const calculation = useMemo(() => {
        ...
        const totalScore = items.reduce((sum, item) => sum + WEIGHT_SCORE[item.weight], 0);
        
        return items.map(item => {
            const share = WEIGHT_SCORE[item.weight] / totalScore;
            const suggestedBlocks = Math.round(share * paceTarget.blocks);
            return {
                ...item,
                share: (share * 100).toFixed(1),
                suggestedBlocks: Math.max(1, suggestedBlocks) // Minimum 1 block
            };
        });
    }, [items, paceTarget.blocks]);
            ...
            <div className="w-full md:w-1/3 bg-slate-900 border border-slate-800 rounded-2xl p-6 h-fit shadow-xl">
                <div className="mb-6 pb-6 border-b border-slate-800">
                    ...
                    <p className="text-sm text-slate-400">
                        Insira as disciplinas do edital. O algoritmo matemático distribuirá seus <strong className="text-white bg-slate-800 px-1 rounded">{paceTarget.blocks} blocos semanais</strong> proporcionalmente ao peso.
                    </p>
                </div>
                ...
                <form onSubmit={handleAddItem} className="space-y-4 mb-6">
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Peso no Edital</label>
                        <select 
                            value={newWeight} 
                            onChange={e => setNewWeight(e.target.value as Weight)}
                            className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white outline-none focus:border-emerald-500 transition-colors"
                        >
                            {Object.values(Weight).map(w => <option key={w} value={w}>{w}</option>)}
                        </select>
                    </div>
                    <button type="submit" className="w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2 border border-slate-700">
                        <Plus size={18} /> Adicionar à Matriz
                    </button>
                </form>

                <div className="space-y-2">
                    {items.map(item => (
                        <div key={item.id} className="flex justify-between items-center bg-slate-950 p-3 rounded-lg border border-slate-800 hover:border-slate-700 transition-colors">
                            <div>
                                <p className="font-bold text-slate-200 text-sm">{item.name}</p>
                                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">{item.weight}</p>
                            </div>
                    ...
                    {items.length === 0 && <p className="text-center text-xs text-slate-600 italic py-4">A lista está vazia.</p>}
                </div>
            </div>

            {/* Results Column */}
            <div className="flex-1 bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col shadow-xl">
                <div className="flex justify-between items-center mb-6 bg-slate-950/50 p-4 rounded-xl border border-slate-800">
                    <div>
                        <h3 className="text-xl font-bold text-white">Resultado do Cálculo</h3>
                        <p className="text-xs text-slate-500">Baseado no ritmo: <span className="text-emerald-500 font-bold">{paceTarget.hours} horas</span> semanais</p>
                    </div>
                    <div className="text-right">
                        <p className="text-xs text-slate-500 uppercase font-bold">Capacidade Utilizada</p>
                        <p className={`text-2xl font-black ${totalCalculatedBlocks > paceTarget.blocks ? 'text-red-500' : 'text-emerald-500'}`}>
                            {totalCalculatedBlocks} <span className="text-sm text-slate-500 font-normal">/ {paceTarget.blocks} blocos</span>
                        </p>
                    </div>
                </div>
                ...
                {items.length > 0 ? (
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-slate-950 text-slate-500 text-[10px] uppercase font-bold tracking-wider sticky top-0 z-10">
                                <tr>
                                    <th className="p-3 rounded-tl-lg">Disciplina</th>
                                    <th className="p-3">Importância Relativa</th>
                                    <th className="p-3 text-right">Blocos Sugeridos</th>
                                    <th className="p-3 text-right rounded-tr-lg">Tempo Semanal</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr key={item.id} className="hover:bg-slate-800/30 transition-colors group">
                                        ...
                                        <td className="p-4">
                                            <div className="flex items-center gap-3">
                                                <div className="flex-1 bg-slate-800 h-1.5 rounded-full overflow-hidden max-w-[100px]">
                                                    <div className="bg-emerald-500 h-full" style={{width: `${item.share}%`}}></div>
                                                </div>
                                                <span className="text-xs text-slate-500 font-mono">{item.share}%</span>
                                            </div>
                                        </td>
                                        <td className="p-4 text-right">
                                            <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-3 py-1 rounded-lg font-bold text-sm">
                                                {item.suggestedBlocks}
                                            </span>
                                        </td>
                                        <td className="p-4 text-right text-slate-400 text-xs font-mono">
                                            ~{Math.round((item.suggestedBlocks * 40) / 60)}h {((item.suggestedBlocks * 40) % 60)}min
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                        ...
                        <div className="mt-6 p-4 bg-emerald-900/10 border border-emerald-500/20 rounded-xl flex gap-4 items-center animate-pulse">
                            <div className="bg-emerald-500/20 p-2 rounded-full text-emerald-500">
                                <ArrowRight size={20} />
                            </div>
                            <div>
                                <h4 className="text-emerald-400 font-bold text-sm mb-0.5">Aplicação Tática</h4>
                                <p className="text-slate-400 text-xs">
                                    Volte para a aba "Visão Tática" e distribua os blocos sugeridos acima ao longo das suas semanas.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-slate-600 opacity-50">
                        <Calculator size={64} className="mb-4 text-slate-700" />
                        <p className="text-sm font-medium">Adicione disciplinas na esquerda para calcular.</p>
                    </div>
                )}
            </div>
        </div>
...
const DraggableCard: React.FC<{ 
  ...
  return (
    <div
      ...
      className={`
        bg-slate-800 border border-slate-700 rounded-xl p-3 transition-all shadow-sm group relative overflow-hidden flex flex-col
        ${isCompact ? 'text-xs' : 'text-sm'}
        ${origin === 'library' ? 'hover:bg-slate-700 hover:border-slate-500 hover:shadow-lg hover:shadow-black/50' : 'hover:bg-slate-800 hover:border-emerald-500/30'}
        ${disabled ? 'opacity-50 cursor-not-allowed grayscale' : 'cursor-grab active:cursor-grabbing hover:shadow-lg'}
        ${isCompleted ? 'opacity-50 grayscale border-slate-800 bg-slate-900' : ''}
      `}
    >
      {origin === 'library' && (
          <div className="absolute right-0 top-0 p-1 bg-slate-900/80 rounded-bl text-[9px] text-emerald-500 uppercase font-bold tracking-tighter opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none border-l border-b border-slate-700">
              Copiar
          </div>
      )}
      ...
      <div className="flex justify-between items-start w-full">
          <div className="flex items-start gap-3 flex-1 min-w-0">
            <div className="mt-0.5 text-slate-600 group-hover:text-emerald-500 transition-colors cursor-grab p-1 hover:bg-slate-700 rounded">
              <GripVertical size={14} />
            </div>
            <div className="flex-1 min-w-0">
              <span className={`inline-block px-1.5 py-0.5 rounded-[4px] text-[9px] font-bold uppercase tracking-wider mb-1 border ${isCompleted ? 'line-through decoration-slate-600 bg-slate-900 text-slate-700 border-slate-800' : 'bg-slate-900 text-slate-400 border-slate-700'}`}>
                {notebook.discipline}
              </span>
              ...
              {/* Performance Stats Resume */}
              <div className="flex items-center gap-2 mt-2">
                 <div className={`text-[10px] font-black flex items-center gap-1 bg-slate-950 px-1.5 py-0.5 rounded border border-slate-800 ${getAccuracyColor(notebook.accuracy, notebook.targetAccuracy)}`}>
                    <TrendingUp size={10} />
                    {notebook.accuracy}%
                 </div>
          ...
          <div className="flex flex-col gap-1 ml-1 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              {/* Toggle Completed Button (Only for Weekly view) */}
              {origin === 'week' && onToggleComplete && (
                  <button 
                    onClick={(e) => { e.stopPropagation(); onToggleComplete(notebook.id, !notebook.isWeekCompleted); }}
                    className={`p-1.5 rounded-lg transition-colors ${isCompleted ? 'text-emerald-500 hover:text-emerald-400 bg-emerald-900/20' : 'text-slate-500 hover:text-emerald-500 hover:bg-slate-700'}`}
                    title={isCompleted ? "Reabrir estudo" : "Marcar como estudado nesta semana"}
                  >
